...
Web content itself is declare-to-use: an app ships the embedded web layer only when app.zon declares it — `"webview"` in `.capabilities`, a `.frontend` block, or a `.shell` webview view — and a manifest that declares none of them builds native-only, where any attempt to create a webview fails with a teaching error instead of loading a layer the app never asked for.
