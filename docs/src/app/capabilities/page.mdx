...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>macOS system-engine host (one player: AVAudioPlayer for local files, AVPlayer for streamed URLs) and Windows system WebView (one Media Foundation media session for local files and streamed URLs); Linux and macOS Chromium report unsupported and playback degrades to one explicit failed Msg</td>
    </tr>
    <tr>
      <td>File drops</td>
