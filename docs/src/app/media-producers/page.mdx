...
```zig
// 1. The model owns the surface id (any nonzero u64 you choose).
const Model = struct {
    player_surface: u64 = 1,
};

// 2. The view binds it (markup: <media-surface surface="{playerSurface}">).
...
- **Paced by the presented-frame clock.** Staged frames are adopted when the compositor's frame ticks (the 60 Hz host timer, or a prompt requested frame). There is no wake channel to manage.
