...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>GPU surface</td>
      <td>Future custom drawing regions for editors, timelines, games, or canvas-like workflows</td>
...
if (runtime.supports(.gpu_surfaces)) {
    try runtime.createView(.{ .label = "canvas", .kind = .gpu_surface, .frame = zero_native.geometry.RectF.init(0, 0, 480, 320) });
}
```
