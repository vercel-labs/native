...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Persisted state: id, label, title, frame, open, focused, hidden, maximized, fullscreen, scale</td>
    </tr>
    <tr>
...
`hide` is supported where the host can re-show a hidden window: macOS (`windowShouldClose` orders the window out) and Windows (`WM_CLOSE` hides via `SW_HIDE`; the tray icon is the re-show affordance). Linux has no status item in this toolkit yet, so nothing could bring a hidden window back — the declaration is refused loudly at build time (a compile error for manifest windows) and at window create (`error.UnsupportedWindowClosePolicy`), never a silent no-op. The hidden state is session-transient: it never persists to the window-state store, so every launch starts shown.
