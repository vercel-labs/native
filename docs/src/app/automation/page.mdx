...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Dispatch a trackpad pinch gesture at a gpu-surface view: the real <code>pinch_begin</code>/<code>pinch_change</code>/<code>pinch_end</code> events, with one change carrying <code>scale - 1</code> so the cumulative gesture scale (the product of <code>1 + delta</code>) lands exactly on <code>scale</code> (1.5 zooms in 50%, 0.5 zooms out to half). The centroid defaults to the view center</td>
    </tr>
    <tr>
