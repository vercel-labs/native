...
| `text`, `badge`, `tooltip` | text leaves | `{}` interpolation allowed in content; `text` takes a line policy via `wrap` (`"true"` word-wraps, `"false"` clips to one honest line) and the typography rungs via `size="heading"`/`size="display"` (themable token steps above title — section headings, hero stats) |
...
| `chart` > `series` | data chart | each series binds a model `[]const f32` iterable via `values="{binding}"` (`kind` is `line`/`area`/`bar`, `color` a token name); `y-min`/`y-max`/`grid-lines`/`baseline` shape the plot — see [Charts](#charts) |
