...
2. **`on_chrome`** delivers the window's chrome overlay insets as a Msg — the bands where OS controls overlay the content (macOS: titlebar height on top, traffic lights on the leading edge; all-zero in fullscreen, on standard-chrome windows, and on platforms without the concept). It fires before the first view build and again when the insets change, so the model pads the header's leading edge instead of hardcoding pixel counts. Main canvas window only (the same scope note as `sync`).
...
macOS-first, like `resizable = false`: GTK and Win32 hosts keep standard chrome (`hidden_inset` is harmless there), report zero insets, and treat drag-region presses as dead space. See `examples/markdown-viewer` for the full retrofit.
