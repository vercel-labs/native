...
An arg can declare a literal default with `name=value`, and a use site may then omit it — `args="title trend=flat"` makes `trend` optional. Defaults are literals only: they evaluate in no scope, so a `{binding}` default is rejected with a teaching error, and a missing arg without a default keeps its error too.
...
```html
<import src="components/board-column.zml"/>
<column gap="12">
  <use template="board-column" title="Todo" cards="{todoCards}" />
</column>
```
...
pub const board_markup_files = [_]canvas.ui_markup.SourceFile{
    .{ .path = "board.zml", .source = @embedFile("board.zml") },
