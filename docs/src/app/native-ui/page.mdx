...
For anything asynchronous — running `gh`, streaming an agent's stdout — `update` never performs the work; it describes it, and results come back as ordinary Msgs. In a TypeScript core the description is `Cmd` data returned beside the next model ([TypeScript Cores: Effects](/typescript#effects-are-cmd-data) has the whole vocabulary); in a Zig core, `update` takes a third parameter, the effects channel — declare `.update_fx` instead of `.update` (two-argument apps are unchanged). The subprocess stream in both:
...
Application-defined native work uses the same slots, completion queue, UI-thread drain, and session journal through `fx.external`. A fixed-width `adapter_id` identifies the application protocol, `kind` identifies an operation within it, and `schema_version` versions that operation's request/result payload contract. The binary-safe payload carries its arguments, keeping the seam directly representable by a future C ABI without teaching Native SDK an application's domain:

```zig
const CoreOperation = enum(u32) { list_items = 1, save_note = 2 };
...
.core_result => |result| model.applyCoreResult(result), // copy result.bytes if the model keeps it
```

`on_result` is required: every accepted request has one terminal lifecycle Msg, including SDK-owned transport failures, so it can retire and replay exactly. Bind the application-owned `ExternalEffectAdapter` once after `UiApp.init` and before the first frame with `app_state.effects.bindExternalAdapter(adapter)`. Its `submit_fn` runs on the UI thread and must only copy/enqueue the borrowed request and return; it must never invoke completion inline. Adapter-owned workers perform the operation and call the supplied thread-safe `completion.complete(.success | .failure, bytes)`. Those are the adapter's only outcomes: cancellation and adapter transport state are SDK-owned and cannot be forged by an adapter. `shutdown_fn` is the ownership barrier: `Effects.deinit` first forwards cancellation for live requests, then calls it; it must not return until no worker can call an old completion. The SDK borrows but never destroys the adapter context.

The seam is deliberately command-sized: request payloads are capped at 64 KiB, result payloads at 256 KiB, at most 16 requests share the ordinary in-flight effect slots, and at most 64 worker results share the completion queue. Deterministic local refusal stays synchronous: `external` returns `ExternalEffectRequestTooLarge`, `ExternalEffectDuplicateKey`, `ExternalEffectPendingFull`, or `ExternalEffectAllocationFailed`, and no request or async result exists. Once a slot and 256 KiB workspace are accepted, `external` returns the request id. A missing adapter or synchronous `submit_fn` refusal then arrives asynchronously as `.adapter_unavailable` or `.submit_failed`, journals normally, and replays without consulting a live adapter. Worker completion only copies into the accepted workspace and never calls the app allocator.

Completion is exact and retryable: an over-bound result returns `ExternalEffectResultTooLarge`, and a full shared queue returns `ExternalEffectQueueFull`; neither consumes the terminal claim or changes the pending request. Retry after space is available, or let the app cancel by key. If a session recorder is active, an over-bound result also fails that recording on the UI drain, loudly and without an end record; a retry cannot make the incomplete journal valid again. Accepted success/failure results and all SDK-owned terminals deliver exactly one async `EffectExternalResult`, carrying the original adapter ID, operation kind, and payload schema version. Cancellation forwards the request id to `cancel_fn` and makes later completion stale. Cancellation, adapter absence, and submit refusal share a separate 16-entry control-terminal queue, one entry per external slot, merged with worker results by global insertion order so none can be evicted or overtaken. A second accepted adapter completion is duplicate. Producer wakes are edge-triggered. The UI thread drains one queue snapshot at a time; work created by handling that snapshot waits for the next batch, whose wake is re-armed exactly once, so recording and replay keep the same event boundary. Only that UI-thread drain creates messages.

Headless tests use the existing fake executor: set `effects.executor = .fake`, inspect requests in issue order with `pendingExternalCount` / `pendingExternalAt`, and answer by SDK `request_id` with `feedExternalResult(request_id, .success | .failure, bytes)`. Recording journals the accepted request ID, adapter ID, key, kind, payload schema version, request-payload SHA-256, terminal result, and delivery order only on external-effect records. Replay parks regenerated fake requests and validates that full identity before feeding each result, without binding or running the live adapter; adapter, key, kind, schema, or request-payload divergence fails replay even when the model fingerprint would not change.
