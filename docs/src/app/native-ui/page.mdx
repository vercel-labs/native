...
`titlebar = "hidden_inset_tall"` is the same shape with the taller unified band: macOS centers the traffic lights in a 52pt band (the tall unified-toolbar look) instead of the compact one — pick it when the header row is toolbar-height so the lights sit on its centerline. Fullscreen falls back cleanly in both variants.
...
A **stepper** is a horizontal stage track (house stepper conventions: indicator + title per step, separators as connectors). The model owns the active index; earlier steps render completed (a check badge), later ones pending:
...
A **timeline** is a ledger list (house timeline conventions: indicator + separator rail + title/description/meta per item). Items are leaves; the attributes carry the content:
