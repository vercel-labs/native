...
The default Native SDK app is native-rendered: the view is a `.zml` markup file, the logic is Zig, and rendering happens through Native SDK's own engine. Scaffold one with `native init my_app` and run it with `native dev`.
