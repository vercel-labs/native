...
Application-defined native work uses the same slots, completion queue, UI-thread drain, and session journal through `fx.external`. A fixed-width `kind` identifies the operation and the binary-safe payload carries its arguments, which keeps the seam directly representable by a future C ABI without teaching Native SDK an application's domain:
