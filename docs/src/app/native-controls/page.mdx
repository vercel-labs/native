...
From a native-rendered app's `update`, the same move is `fx.focusView(window_label, view_label)` — the effect-channel route to `runtime.focusView`, which a pure `update` cannot reach directly. It is fire-and-forget like the window verbs: an unknown window or view label is a no-op, and the fake executor records the request in `windowActionState()` (`focus_view_count`, `lastLabel()`, `lastViewLabel()`) so hermetic tests can pin it. This is the seam behind app-owned pane cycling — a shell that mounts a canvas beside a `<terminal>` and a WebView owns focus policy itself, because the platform key-view loop does not span them and a focused terminal deliberately keeps Tab for its child.
