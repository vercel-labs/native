...
<dl>
  ...
  <dd>Authoring <a href="/typescript">TypeScript app cores</a>: the app-core subset, every checker rule by ID with its idiomatic fix, text-is-bytes, the full <code>Cmd</code>/<code>Sub</code> effect vocabulary, and the node dev loop.</dd>
  ...
  <dt><code>zig</code></dt>
  <dd>Zig 0.16 idioms indexed by compile error, for fixing code written against older Zig: <code>std.Io</code> file IO and writers, <code>ArrayList</code>, spawning, clocks, formatting, and <code>build.zig</code> shapes, each as the SDK writes them. The human version is <a href="/zig">Zig 0.16 Notes</a>.</dd>
</dl>
