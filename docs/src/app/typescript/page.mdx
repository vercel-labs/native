...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>Cmd.imageLoad(id, source, &#123; event &#125;)</code> + <code>imageCancel(id)</code></td>
      <td>Load an image at runtime under the model-owned numeric ImageId your markup binds; one <code>event</code> result — <code>loaded</code> with the decoded width/height, or a failure class. <code>imageCancel</code> ends a live load loudly (state <code>cancelled</code>) and frees the id — see <a href="/dynamic-images">Dynamic Images</a></td>
    </tr>
    <tr>
