...
For a single panel with an engine-managed drag handle (no fraction bookkeeping), use `resizable`: `width` sets the initial width and the engine owns the drag from there. It is a stacking surface — no `gap`; put a row or column inside.
...
<AttrTable attrs={["value", "min-width", "gap", "on-resize", "width"]} />
