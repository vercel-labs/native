...
A text-bearing press control. The label is the element's content, `on-press` dispatches its Msg, and an inline `icon` draws before the label as part of the same hit target. Six variants cover the shadcn palette; previews are rendered by the engine itself.
...
`button-group` is a row container that visually attaches its buttons; drive the active segment with `selected`.

<ComponentPreview name="button-group" alt="A button group rendered by the engine" />

```html
<button-group>
  <button variant="outline" selected="{range == years}" on-press="show_years">Years</button>
  <button variant="outline" selected="{range == months}" on-press="show_months">Months</button>
  <button variant="outline" selected="{range == days}" on-press="show_days">Days</button>
</button-group>
```
