...
A text-bearing press control. The label is the element's content, `on-press` dispatches its Msg, and an inline `icon` draws before the label as part of the same hit target. Six variants cover the token palette; previews are rendered by the engine itself.
