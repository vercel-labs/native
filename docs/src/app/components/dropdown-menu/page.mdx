...
Handle the item's Msg in `update` and clear the open flag there — a menu press does not dismiss implicitly. [Select and combobox](/components/select) build their option lists from exactly this pattern (with `anchor-alignment="stretch"` for the select-menu width).
...
Native right-click context menus (macOS `NSMenu`) are a separate, builder-only channel: the `context_menu` option on any element presents OS menu items whose selection dispatches a Msg — see [Menus](/menus).
