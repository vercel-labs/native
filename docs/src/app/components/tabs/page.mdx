...
A row container for a tab strip. The children are [buttons](/components/button) whose `selected` state marks the active tab — usually an equality against a model field — and whose `on-press` switches it. The segmented-control foundation is what the engine renders tab strips with (the builder uses `segmented_control` children directly). The panel itself is ordinary conditional content: an `if` per tab under the strip.
