...
Value controls: the label is the element's content, the model binds `checked`, and `on-toggle` dispatches its Msg — the engine never flips state on its own. `radio` is the single-selection variant, grouped by a `radio-group` row container; render the group's selection with `{a == b}` equalities and let each radio's `on-toggle` set the model field.
...
```html
<radio-group gap="12">
  <radio checked="{density == default}" on-toggle="set_default">Default</radio>
  <radio checked="{density == comfortable}" on-toggle="set_comfortable">Comfortable</radio>
