...
Native controls can also bind a `command` when created with `runtime.createView(...)` or `window.zero.views.create(...)`. On macOS system WebView builds, native button views dispatch the command with the native `window_id` and the originating view label. Controls inside a toolbar report `source = .toolbar`; other native controls report `source = .native_view`.
...
View commands are available through `window.zero.views.*` when `js_window_api` is `true`. They use the same origin and `window` permission checks as window and WebView commands, and `windowId` must match the calling window when provided. `kind: "webview"` routes through the WebView backend. The macOS system-WebView backend supports native `toolbar`, `titlebar_accessory`, `sidebar`, `statusbar`, `button`, `checkbox`, `toggle`, `text_field`, `search_field`, `label`, and `spacer` views; other platform hosts and native kinds return explicit unsupported-backend errors until implemented.
