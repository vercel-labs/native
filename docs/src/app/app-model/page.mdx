...
Compiled markup views compose the other way too: a hand-written Zig builder root can build markup fragments as ordinary children. This is the pattern for charts-in-markup apps and any UI that mixes custom Zig panes with declarative markup — the root places what the closed grammar cannot express (a `ui.chart` pane, a scaled display block), and the markup keeps everything it can:
...
pub fn rootView(ui: *Ui, model: *const Model) Ui.Node {
    return ui.column(.{ .gap = 12, .grow = 1 }, .{
        ...
        ui.chart(.{ .grow = 1, .height = 220, .y_min = 0 }, &.{
            .{ .kind = .line, .values = model.cpuHistory(), .fill = true, .color = .accent, .label = "cpu" },
        }),
    });
}
