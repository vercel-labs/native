...
Fresh windows use three-way placement. A successful state-store lookup restores the saved frame. Otherwise, an authored `x` or `y` places the window at that origin (constrained to a visible screen on macOS). With neither saved state nor an explicit origin, macOS centers the main window; secondary windows cascade under the default `clamp_to_visible_screen` policy, while `center_on_primary` keeps them centered. `restore_state = true` is only the persistence opt-in—it no longer makes a first launch look like a restored frame at `(0, 0)`.
