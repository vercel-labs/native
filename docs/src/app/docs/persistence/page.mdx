...
Increase `.persist.version` whenever the `Model` shape changes, and never reuse a version. `native check` remembers the last accepted version/fingerprint pair under `.native/cache` and reports NS1065 when the shape changes without a bump or the version moves backward. The snapshot header carries that app version, the generated model-only shape fingerprint, and the compiler's snapshot-format version, so a cold checkout still fails closed at runtime: a same-version shape mismatch reports `corrupt`, and a snapshot from a future app version reports `version_unknown`.
