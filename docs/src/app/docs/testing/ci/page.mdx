...
```bash
native automate assert 'gpu_nonblank=true' 'role=button name="Reset"' 'total: 0'
```
...
4. `automate assert` proves the GPU surface presented non-blank pixels and the scaffolded counter UI (the Reset button and the `total: 0` status bar) reached the accessibility snapshot.
