...
Static images that ship with the app stay in [`app.zon`'s `.assets.images`](/docs/app-zon) — read once at launch, registered before the first frame. This page is the **dynamic** half: cover art fetched from a CDN, a chart PNG a subprocess rendered, an avatar that arrives after login.
