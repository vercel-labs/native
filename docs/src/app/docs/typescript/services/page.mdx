...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Explicit opt-in: native Linux, cross-Linux x86_64/aarch64, Windows x86_64, or macOS built on macOS</td>
    </tr>
  </tbody>
</table>
...
- **Scope.** Services are desktop-only today. Child executables follow the pinned compiler's broad matrix: same-platform builds, Linux and Windows targets cross-compiled from a macOS/Linux/Windows host, and macOS targets built on macOS. In-process archives use the compiler's narrower object-localization matrix: native Linux, cross-Linux x86_64/aarch64, Windows x86_64, or macOS built on macOS. A pairing outside the relevant matrix fails with a teaching, as does a Linux `-gnu` target spelled without a glibc version — the service runtime needs glibc 2.36+ or musl, so cross Linux targets are spelled `x86_64-linux-gnu.2.36` (or later) or `x86_64-linux-musl`. Operations are synchronous.
