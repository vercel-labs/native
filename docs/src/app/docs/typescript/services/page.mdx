...
Services run on a supervised carrier — compiled into the app binary on a worker-thread pool by default, or as a separate child process — and are desktop-only today (see [Runtime behavior](#runtime-behavior)).
...
| Carrier | Where services run | Default on |
| --- | --- | --- |
| `in_process` | Compiled into the app binary; a small thread pool, one isolated instance per thread | Host-native macOS and Linux builds |
| `child` | A second native executable — `<app>_services` — beside the app binary, packaged with it | Windows; any build the in-process carrier cannot serve |

`.service_carrier = "in_process" | "child"` in app.zon (or `-Dservice-carrier`) states the choice; unset means the supported default. `.service_pool_size` (or `-Dservice-pool-size`, 1-16) sets the in-process pool width; the default is min(4, cores).
...
- **Scope.** Services are desktop-only today and build for the build host: a cross-target build of a service-bearing app fails with a teaching. Operations are synchronous.
