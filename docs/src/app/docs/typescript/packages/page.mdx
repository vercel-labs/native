...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>A native service-host process, reached with <code>Cmd.request</code></td>
    </tr>
    <tr>
...
Put ordinary static-tier TypeScript under `src/services/` when the work needs Node built-ins, regexes, JSON, `Map`/`Set`, `Date`, classes, or ambient process authority. Each directly exported synchronous function becomes an operation named `<module-basename>.<export>` and takes zero arguments or one `Uint8Array`, returning `Uint8Array`. The core calls it with `Cmd.request`, so success and failure still arrive as Msgs and record/replay remains offline.

This is compiled native code with no JavaScript engine. Phase 1 is vendored-source-only: bare package imports teach NS1066, so check the utility's source in under `src/services/`. The host is a lazily spawned sibling executable on desktop today; [TypeScript services](/docs/typescript#typescript-services) covers authority, crash recovery, packaging, and the mobile/in-process limitation.
...
- **Vendor ordinary static-tier code under `src/services/`.** Classes, regexes, JSON, `Map`/`Set`, `Date`, Node built-ins, and imperative transforms stay in TypeScript and compile into the service host; the core reaches their exported byte operations through `Cmd.request`.
...
One thing deliberately does not exist: a package manager for cores. A core's import graph is exactly its class under `src/` plus the SDK modules. Phase-1 services are hermetic too: their source is checked in under `src/services/`, with no install or network step. Nothing arrives at build time that you have not checked in.
