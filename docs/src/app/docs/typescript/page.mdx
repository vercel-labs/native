...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>Cmd.credentialSet(...)</code> / <code>credentialGet(...)</code> / <code>credentialDelete(...)</code></td>
      <td>Routed access to Keychain, Secret Service, or Credential Manager; get returns the secret bytes and missing items route <code>not_found</code></td>
    </tr>
    <tr>
...
A service operation is a directly exported, non-default named synchronous function under `src/services/`. Phase 1 keeps the boundary deliberately small: zero arguments or one `Uint8Array` payload, and a `Uint8Array` result. The operation name is `<module-basename>.<export>`; `native check` emits `services.contract.json` internally and rejects unknown literal names before the build.
...
export function parse(payload: Uint8Array): Uint8Array {
  ...
  seen.set("parsedAt", Date.now());
  const source = new TextDecoder().decode(payload);
  return new TextEncoder().encode(JSON.stringify({
    matches: /feed/i.test(source),
    cwdExists: fs.existsSync("."),
    facts: seen.size,
  }));
}
```
...
case "parse":
  return [
    model,
    Cmd.request("feeds.parse", model.source, {
      key: "parse",
      ok: "parsed",       // one Uint8Array field
      err: "parse_failed", // one Uint8Array field
    }),
  ];
```

The core never receives a synchronous handle. Its update returns a command, the service result crosses back as the named Msg arm, and the runtime journals that result like every other effect. Replay parks the request and feeds the recorded bytes without launching the service executable.

Service modules are ordinary TypeScript on scriptc's static tier: Node built-ins such as `fs`, `path`, `process`, `os`, and `child_process`, plus `fetch`, regexes, JSON, `Map`/`Set`, `Date`, and classes are available when the pinned compiler supports them. They may import other vendored service files and subset-legal core-class files. Bare package specifiers remain NS1066 in this phase — check source in under `src/services/`; hermetic npm sources arrive in a later phase. Async functions and record payloads also wait for the typed-boundary phase.
...
Today the carrier is a lazily started second executable placed next to the app executable. Startup verifies both the protocol version and a fingerprint of the generated operation registry, so a stale or mismatched sibling is rejected before an operation index is dispatched. It restarts after a crash; the request interrupted by the crash receives its error arm, and the next request starts a fresh host. This makes TypeScript services desktop-only and adds a process hop plus two payload copies. The current compiler emits the service executable only for the build host, so cross-target service builds fail with a teaching instead of packaging an executable for the wrong OS or architecture. The future in-process carrier keeps the same contract and app API; no JavaScript engine is used in either form.

## The dev loop

`native dev --core` is the fastest loop for logic work: the core runs under node with a virtual host — dispatch Msgs as JSON lines (`{"kind":"add"}`, `{"$bytes":"…"}` for bytes payloads), advance a virtual clock (`{"advance":1000}`) to fire timers deterministically, and watch the committed model and effect transcript. Effects the virtual host does not perform — including service requests — print as `cmd ...` lines; feed their results back yourself as ordinary Msg lines. Pair `--script msgs.ndjson` with `--watch` to replay a scenario on every edit. `native dev` runs compiled services for real beside the native app. Full service simulation in the virtual devhost is a later phase. [Quick Start](/docs/quick-start#the-fastest-loop-the-core-under-node) shows a full transcript.
...
The complete core guide ships as `native skills get ts-core`; the service contract, authority posture, and phase-1 limitations ship as `native skills get ts-services`. Both are written for AI agents and precise enough for humans.
