...
A service operation is a directly exported, non-default named synchronous function under `src/services/`. It takes zero or one explicitly typed request and declares a contract-encodable result: booleans, numbers, proven integer-class fields, bytes, optionals, readonly slices, and named records/enums/kind-unions. Put crossing shapes in an exported, subset-legal module outside `src/services/` so the core and service import one declaration. The operation name is `<module-basename>.<export>`; `native check` projects its complete type table into `services.contract.json`, checks both classes, and generates the core client.
...
export function parse(request: ParseRequest): ParseResult {
  ...
  return { bytes: new TextEncoder().encode(JSON.stringify({
    escaped: escapeStringRegexp(source),
    matches,
...
`@native-sdk/services` is generated from the sidecar. Its constructor encodes the request and conditionally proves that `ok` names the one Msg arm carrying `ParseResult`; a stale field or wrong route is a `native check` type error. The generated source lives only in build/dev scratch space and the ignored editor package under `node_modules`, never in authored `src/`. Raw `Cmd.request` remains the low-level bytes form.
