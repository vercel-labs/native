...
Inside a Native SDK app, you mostly don't need `std.Io` at all: `update` does files, subprocesses, HTTP, and timers through the [typed effects channel](/docs/state) (`fx.readFile`, `fx.spawn`, `fx.fetch`, `fx.startTimer`), and clock reads go through `fx.wallMs()` or `native_sdk.nowMs()`. Raw `std.Io` shows up in `main`, in tests, and in standalone tools.
...
App code should not reach for either: `native_sdk.nowMs()` and `native_sdk.monotonicMs()` are the SDK's clock facade, `fx.wallMs()` is the journaled read inside `update`, and repeating work belongs to `fx.startTimer`. See [State & Data Flow](/docs/state).
