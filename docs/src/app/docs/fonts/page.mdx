...
The SDK bundles two faces — Geist Regular and Geist Mono — and every built-in component renders through them by default. Bundled coverage is Latin-script: a codepoint outside it (CJK, emoji, most symbols) uses the face's glyph-0 (`.notdef`) fallback — normally tofu, but intentionally no ink if that glyph is empty or unusable. Registering a font is how an app renders a script the bundled faces do not cover: register a face that covers the characters, point the typography tokens at it, and both renderers ink the registered outlines. Chinese and Japanese text render this way today.
...
Codepoints the registered face does not cover keep the same per-glyph fallback as the built-ins: the face's glyph-0 (`.notdef`) outline. It normally renders visible notdef, but intentionally leaves no ink when glyph 0 is empty or unusable. The engine never silently cascades into another family — the reference renderer, the glyph atlas, and packet text through the SDK's rasterizer use that selected-face fallback rather than an approximation. Text the macOS host draws follows platform shaping, including system-font fallback, so uncovered codepoints there may render from a substituted family instead of notdef.
