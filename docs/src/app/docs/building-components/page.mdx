...
When the closed markup vocabulary does not expose a lower-level engine capability, keep the TypeScript core and extend only the view tier with a Zig view function over `canvas.Ui`. This is the app-side escape hatch for shapes such as `data_grid` cell renderers or lower-level floating surfaces; it is toolkit-extension work, not a reason to migrate the app's core. Prefer markup while its grammar covers the component, because templates keep `native check` and hot reload automatic.
...
**Build it.** When the library has no shape for it, compose one from markup primitives — the stat card and empty-state above — or use a Zig view function when the closed markup grammar cannot express the shape. Your component then themes, tests, and automates through the same widget tree as a built-in.
