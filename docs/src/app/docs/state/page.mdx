...
Bindings are zero-argument by design. A parameterized query (the cards of column X) becomes one model function per case, or a [template](/docs/native-ui#templates) arg.
...
`on-scroll` follows the same shape with a `canvas.ScrollState` payload — store `offset`, echo it through the scroll's `value`, and slide a bounded window from it when content is large. See [Native UI: Messages](/docs/native-ui#messages).

## Time: a testable seam

In a TypeScript core this is settled by rule: `update` is deterministic, and time arrives as message payloads (`Cmd.now`, `Cmd.delay`, `Sub.timer` — see [TypeScript Cores](/docs/typescript#effects-are-cmd-data)). The seam below is the Zig core's equivalent. Zig 0.16 puts `std.time` behind `std.Io`, which `update` never sees — do not call `clock_gettime` yourself. The facade owns the clocks:
