...
The `app.json` manifest declares app metadata, permissions, security rules, window layout, and packaging inputs. It is read by the CLI and tooling at build, package, and validation time. New projects use JSON so TypeScript developers get familiar syntax, completion, and inline validation through [`$schema`](https://native-sdk.dev/schemas/app.schema.json). Existing `app.zon` manifests remain fully supported and expose the same fields—there is no reduced JSON feature set.
