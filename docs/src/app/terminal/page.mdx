...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Terminate the child's job (SIGKILL to its process group — the whole foreground job; a descendant that re-grouped itself escapes, POSIX offering no kill-whole-session primitive). The exit then reports <code>cancelled</code>.</td>
    </tr>
  </tbody>
</table>
