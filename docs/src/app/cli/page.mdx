...
Write an owned `build.zig`/`build.zig.zon` into the app (once); the verbs then drive your files via `zig build`.
...
<dl>
  ...
  <dd>Validate Native markup without building; errors carry <code>file:line:column</code> and a teaching message (files still on the former <code>.zml</code> extension check too, with a rename note). Run inside an app directory it picks up a fresh model contract the same way <code>native check</code> does; <code>--strict</code> promotes warnings to failures.</dd>
...
List and print the built-in AI agent skills the CLI ships. See [Agent Skills](/skills) for what each skill covers and how to deliver one to an agent.
