...
`on-input` names a `Msg` variant whose payload is `canvas.TextInputEvent`; `canvas.TextBuffer` applies each event and holds the text. Clipboard and selection come free: the runtime owns cmd/ctrl+C/X/V in editable text, cut and paste arrive as ordinary edit events, and the mirror stays consistent with zero extra code.
