...
Carrier selection is a build fact: `.service_carrier = "in_process" | "child"` in app.zon (or `-Dservice-carrier`). Unset/`"auto"` preserves the child carrier; macOS, Linux, and Windows apps can explicitly opt into `"in_process"` wherever the pinned compiler's build matrix covers the host/target pairing. `.service_pool_size` (or `-Dservice-pool-size`, 1–16) sets the in-process pool width; the default is min(4, cores).
...
- desktop targets only, on the pinned compiler's build matrix: same-platform builds always work, Linux and Windows targets cross-compile from any macOS/Linux/Windows build host, macOS targets need a macOS host, and a cross Linux `-gnu` target must spell its glibc (`x86_64-linux-gnu.2.36` or later, or `-musl`) — the service runtime's floor;
