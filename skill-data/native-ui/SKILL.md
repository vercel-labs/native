...
Deletion is platform-correct and semantic: Backspace/Delete remove one caret unit, Option+Backspace/Delete on macOS (Ctrl+Backspace/Delete elsewhere) remove one word, and Command+Backspace on macOS emits `delete_to_line_start`. That event removes the active selection when non-empty; otherwise it removes caret-to-hard-line-start (offset 0 in single-line fields, the current newline-delimited line in a textarea, with CRLF atomic). Soft-wrapped textarea lines intentionally use the hard line start in v1. Apply the event through the same `TextBuffer` / TypeScript `applyTextInputEvent` mirror — it is one edit and therefore one undo step.
