...
- `Cmd.credentialSet(service, account, secret, { key?, ok, err })` / `credentialGet(service, account, route)` / `credentialDelete(service, account, route)` — Keychain on macOS, Secret Service/libsecret on Linux, Credential Manager on Windows. Every identifier/secret is bytes. Set/delete deliver empty bytes on `ok`; get delivers the secret bytes; missing get/delete routes `err` with `not_found`. Other err reasons are `invalid_request`, `unsupported`, and `failed`. Results journal and replay like `Cmd.request`.
...
Export `statusItem(model): StatusItemState` from `src/core.ts` to make the generated launcher install a native menu-bar status item and keep its presentation and menu synchronized with committed model state. Presentation and menu hash independently: changing title/width/tone/icon opacity/number style does not rebuild the rows, and neither change recreates the native item. `statusItem` is launcher-bound automatically, so do not repeat it in `viewUnbound`.
...
The canonical records live in `@native-sdk/core/events`. Install-time fields are `iconPath`, `tooltip`, `activationCommand`, `alternateActivationCommand`, and `openCommand`; all commands route through `commandMsg(name): Msg | null`. Presentation is exact: `title`, `width` (0 = host default), `tone` (`normal | warning | critical`), `iconOpacity` (0…1), and `monospaced`. Every row has id/label/command/separator/enabled plus byte `detail`, `role` (`command | info | header | hero | agent | context`), byte `key`, and the five explicit modifier booleans. Actionable rows need unique non-zero whole u32 ids and a non-empty label, separators conventionally use id 0, and the runtime cap is 32 rows. Row membership and every live presentation/menu field may derive from the model. Linux does not implement status items today.
...
const label = utf8Bytes(`${done} of ${total} done`); // arena UTF-8 bytes
...
- **`statusItem(model: Model): StatusItemState`** — the complete menu-bar item: install-time icon/tooltip/click hooks plus live presentation and dropdown. The generated launcher installs it and re-derives the live parts after committed updates; import the exact records from `@native-sdk/core/events`.
...
- **NS1018 no string concatenation.** `+` (or `+=`) with a string operand builds text at runtime; build display bytes with `utf8Bytes` on a template literal, or stitch `Uint8Array` buffers with `.set`.
...
- **NS1024 model text is bytes.** A `string`-typed field anywhere in the Model tree has no commit representation (it would need a JS string heap). Type it `Uint8Array` and build display values with `utf8Bytes`, or use a string-literal union when the field holds one of a closed set of tags. Reported at the field's declaration.
...
- `update`/`update_fx`/`init_fx` belong to the adapter (it runs your `initialModel`, `update`, and `subscriptions` through the effect bridge), and the adapter wires `on_command`/`on_key`/`on_pinch`/`on_drop`/`on_appearance`/`on_chrome`/`on_frame` from the core's channel exports automatically. It likewise wires `theme_fn` from `themePack(model)` and `status_item_fn` from `statusItem(model)`. A wiring that also sets one of those seams is a teaching panic. Everything else is ordinary wiring: `tokens_fn`/`windows_fn` derive from `*const core.Model`, a static `status_item` can supply icon/tooltip, and `CoreOptions` carries the adapter-owned knobs (`audio_cache_dir`, `boot_images`, `env_values`).
