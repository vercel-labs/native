...
/// Keeping the registry file-shaped gives the generated launcher a closed,
/// comptime-compiled view set while `windows(model)` owns dynamic liveness.
fn collectTsWindowViews(b: *std.Build, app_root: []const u8) []const TsWindowView {
    const windows_path = appPath(b, app_root, "src/windows");
    var dir = b.build_root.handle.openDir(b.graph.io, windows_path, .{ .iterate = true }) catch return &.{};
    ...
    while (walker.next(b.graph.io) catch null) |entry| {
        ...
        if (std.mem.indexOfScalar(u8, entry.path, '/') != null or std.mem.indexOfScalar(u8, entry.path, '\\') != null) {
            @panic("\nTypeScript secondary-window views live directly under src/windows/: move nested .native files to src/windows/<label>.native\n");
        }
...
fn tsWindowRegistrySource(b: *std.Build, views: []const TsWindowView) []const u8 {
    ...
    for (views, 0..) |view, index| {
        const line = std.fmt.allocPrint(
            b.allocator,
            "const WindowView{d} = canvas.CompiledMarkupView(core.Model, core.Msg, @embedFile(\"{f}\"));\n",
            .{ index, std.zig.fmtString(view.staged_path) },
        ) catch @panic("OOM");
...
fn tsCoreStage(
    ...
    for (window_views) |view| {
        _ = staged.addCopyFile(b.path(appPath(b, app_root, view.source_path)), view.staged_path);
    }
