...
pub fn build(b: *std.Build) void {
    ...
    const cli_bin = b.pathJoin(&.{ zero_native_path, "zig-out/bin/zero-native" });
    const cli_build = b.addSystemCommand(&.{ "sh", "-c", b.fmt("cd '{s}' && zig build", .{zero_native_path}) });

    const dev = b.addSystemCommand(&.{ cli_bin, "dev", "--manifest", "app.zon", "--binary" });
    dev.addFileArg(exe.getEmittedBin());
    dev.step.dependOn(&exe.step);
    dev.step.dependOn(&frontend_install.step);
    dev.step.dependOn(&cli_build.step);
    ...
    package.step.dependOn(&cli_build.step);
