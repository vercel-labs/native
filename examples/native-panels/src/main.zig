...
const NativePanelsApp = struct {
    ...
    fn event(context: *anyopaque, runtime: *zero_native.Runtime, event_value: zero_native.Event) anyerror!void {
        ...
        switch (event_value) {
            ...
            .appearance_changed, .shortcut, .timer, .effects_wake, .files_dropped, .gpu_surface_frame, .gpu_surface_resized, .gpu_surface_input, .canvas_widget_pointer, .canvas_widget_keyboard, .canvas_widget_file_drop, .canvas_widget_drag, .lifecycle => {},
        }
    }
