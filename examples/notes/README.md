...
`native test` (or root `zig build test-example-notes`) drives the real dispatch paths: title/snippet/relative-time derivation, store round-trips (including garbage, header-only, and orphaned-note inputs), edit → debounce timer → serialized write → pending-save re-persist through the fake effect executor, boot-time restore into the widget tree, the dialog flow through automation `widget-click` (create, duplicate-name hint, rename prefill, scrim click-away, capacity), the whole shortcut map through platform shortcut events, live search filtering and the built-in clear affordance through raw pointer events, folder/note row clicks through the pressable list rows, compiled/interpreter markup parity with the dialog closed and open, the system appearance re-deriving the tokens live through platform events, the controlled note-list scroll round-trip, and three-pane layout at window size.
