...
pub const MacPlatform = struct {
    ...
    pub fn deinit(self: *MacPlatform) void {
        ...
        if (self.channel_wake_abandoned.load(.seq_cst)) {
            std.debug.print("macos platform teardown: an abandoned channel wake call may still enter this host; skipping destruction and leaking it, process-lived, so the stale call stays safe\n", .{});
            return;
        }
