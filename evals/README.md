...
3. **Grades** with deterministic checks: `zig build test` in the workspace, `native markup check` on the `.zml` files, per-case file greps (e.g. "the board uses `<template>`"), and live automation-snapshot greps (build with `-Dautomation=true`, launch, `native automate wait`, grep `snapshot.txt` for expected roles/names).
...
- `metrics-dashboard` — exercises the chart widget and icon-in-button, including the markup/Zig seam: markup owns the chrome (header with `<button icon=...>`, status bar) while the latency plot must be `ui.chart` composed in as a Zig subtree (markup bindings are scalar, so this is the documented shape). Greps assert `.chart(`, `icon="`, and the pinned `y_min = 0`; the snapshot asserts `role=chart` plus the seeded summary.
