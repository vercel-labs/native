...
/// The twelve text-input event tags the markup engines recognize
...
/// engines route through the dedicated ABI entry: exactly the twelve tags,
...
test "a text-input-named union without the payload shapes rides the record entry" {
    ...
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    // Twelve right names, wrong insert_text payload (void): the markup
