...
pub const attribute_docs = [_]Doc{
    ...
    .{ .name = "resize-duration", .doc = "split only: layout-tween duration in milliseconds (a plain number or one {binding}). Nonzero makes the bound value a TARGET - a rebuild that moves it eases the rendered fraction there, one step per presented frame, reflowing both panes like a divider drag and dispatching the same on-resize echoes. 0 (and absent) snaps, today's behavior. Reduced-motion appearances snap automatically - apps declare nothing extra." },
...
pub const event_docs = [_]Doc{
    ...
    .{ .name = "on-submit", .doc = "Dispatch a Msg on enter in a text field (primary+enter in a textarea): tag or tag:{payload}." },
