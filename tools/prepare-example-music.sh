...
# Strip, retag, and verify one track. Prints its duration in ms.
#
...
for entry in "${CATALOG[@]}"; do
    ...
    while IFS= read -r file; do
        ...
        duration_ms="$(prepare_track "$SRC_ROOT/$folder/$file" "$OUT_ROOT/$album_slug/$track_slug.mp3" \
            "$title" "$artist" "$album" "$year")"
        tracks_zon+="$(printf '                .{ .title = "%s", .file = "%s", .duration_ms = %s },' \
            "$(zon_escape "$title")" "$(zon_escape "$dst_rel")" "$duration_ms")"$'\n'
        total_tracks=$((total_tracks + 1))
        echo "  $artist / $album: $title (${duration_ms}ms)"
